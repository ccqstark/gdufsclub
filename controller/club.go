...
func RegisterClub(c *gin.Context) {
	...
	//二合一表信息提取
	var club model.Club
	club.ClubName = c.PostForm("club_name")
	club.ClubEmail = c.PostForm("club_email")
	club.ClubWechat = c.PostForm("club_wechat")
	club.ClubPhone = c.PostForm("club_phone")
	club.ClubAccount = c.PostForm("club_account")
	club.ClubPassword = c.PostForm("club_password")
	var err error
	club.TotalProgress, err = strconv.Atoi(c.PostForm("total_progress"))
	if err != nil {
...
func UploadClubLogo(c *gin.Context) {
	...
	//插入数据库
	if ok := model.UpdateLogo(clubID.(int), fileNameExt); ok == true {
		c.JSON(http.StatusOK, gin.H{
			"code": 200,
			"msg":  "上传成功!",
			"result": gin.H{
...
//导出当前所有面试者excel
func GetExcelAll(c *gin.Context) {
	...
	//获取此面所有人的user_id
	passerID := model.QueryAllPerson(clubID.(int), progress)
